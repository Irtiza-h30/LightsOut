...
public class LightsOut {
	...
	public static ArrayList<Solution> solve(int width, int height){
		...
		return solveLoop(q, new GameModel(width,height));
	}

	public static ArrayList<Solution> solveLoop(Queue<Solution> q, GameModel model){
	...
	public static Solution solveShortest(GameModel model){
		...
		if (results.size()!=0){
			int shortest=results.get(0).getSize();
			int shortestIndex=0;
			for (int i=1;i<results.size();i++){
				if (results.get(i).getSize() < shortest){
					shortest=results.get(i).getSize();
					shortestIndex=i;
				}
			}
			s=results.get(shortestIndex);
		}
		return s;
	}
	...
	public static void main(String[] args) {
		...
		ArrayList<Solution> results   = solve(new GameModel(width,height));
